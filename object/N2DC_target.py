...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            ...
            netF.eval()
            netB.eval()
            mem_label = obtain_label(dset_loaders['test'], netF, netB, netC, args, iter_num_update)
            mem_label = torch.from_numpy(mem_label).cuda()
            feas_all = obtain_all_fea(dset_loaders['test'], netF) # get the features of the all-target-data 
            netF.train()
            netB.train()
        ...
        features_test_N = obtain_nearest(features_test_F, feas_all)
...
def obtain_label(loader, netF, netB, netC, args, iter_num_update_f):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
    ...
    all_idx_nn = get_nearest_sam_idx(all_fea_F, all_fea_F) 
    ln = all_label.shape[0]
...
def obtain_nearest(data_q, data_all):
    ...
    # sam_nearest = get_nearest_sam_idx(data_q_, data_all_)
    # re = torch.from_numpy(sam_nearest)
    nearest_idx = get_nearest_sam_idx(data_q_, data_all_)
    re = data_all[nearest_idx, :]
    return re # array

def get_nearest_sam_idx(Q, X): # Q、X arranged in format of row-vector
    Xt = np.transpose(X)
    Simo = np.dot(Q, Xt)               
    ...
    # X_nearest = X[indices_min_second, :]
    # return X_nearest
    return indices_min_second


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='N2DC')
    ...
    parser.add_argument('--output', type=str, default='ckpsglx2020r7/target_geometry_near/')
    parser.add_argument('--output_src', type=str, default='ckpsglx2020r7/source/')
